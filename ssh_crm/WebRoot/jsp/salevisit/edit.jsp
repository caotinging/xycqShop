...
<BODY>
	<FORM id=form1 name=form1 action="${pageContext.request.contextPath }/saleVisitAction_updateSv" method="post" >
		<!-- 获取当前登录用户 -->
		<input type="hidden" name="user.user_name" value="${saleVisit.user.user_name }">
		<input type="hidden" name="user.user_id" value="${saleVisit.user.user_id }">
		<!-- 当前修改的记录的id -->
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							<TR>
								<td>所属客户：</td>
								<td>
									<input type="hidden" id="cust_id_Hbtn" name="customer.cust_id" value="${saleVisit.customer.cust_id }"> 
									<input type="text" name="customer.cust_name" style="WIDTH: 180px" id="cust_name_Btn" value="${saleVisit.customer.cust_name }" /> 
								...
								<td>拜访时间 ：</td>
								<td>
									<INPUT class=textbox id="visit_time" type="text" style="WIDTH: 180px" maxLength=50 name="visit_time" readonly="readonly" value="${saleVisit.visit_time_s }" />
								</td>
							</TR>
							<TR>
								<td>被拜访人 ：</td>
								<td>
									<input type="hidden" id="lkm_id_Hbtn" name="linkMan.lkm_id" value="${saleVisit.linkMan.lkm_id }"> 
									<input type="text" name="linkMan.lkm_name" style="WIDTH: 180px" id="lkm_name_Btn" value="${saleVisit.linkMan.lkm_name }" /> 
									<input type="button" style="cursor: pointer;" value="选择联系人"
									onclick="selectLinkMan()">
								</td>
								<td>拜访地址：</td>
								<td>
									<INPUT class=textbox id=sChannel2 style="WIDTH: 180px" maxLength=50 name="visit_addr" value="${saleVisit.visit_addr }">
								</td>
							</TR>
							...
							<TR>
								<td>拜访详情 ：</td>
								<td>
									<INPUT class=textbox style="WIDTH: 180px" maxLength=50 name="visit_detail" value="${saleVisit.visit_detail }">
								</td>
								<td>下次拜访时间：</td>
								<td>
									<INPUT class=textbox id="visit_nexttime" readonly="readonly" style="WIDTH: 180px" maxLength=50 name="visit_nexttime"  value="${saleVisit.visit_nexttime_s }">
								</td>
							</TR>
							<tr>
								<td rowspan=2>
