...
<script type="text/javascript">
	$(function(){
		...
		$('#save').click(function(){
			var res = $("#userForm").form('validate');
			if(res){
				$("#userForm").submit();
			}
		});
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div region="center" style="overflow:auto;padding:5px;" border="false">
		<form id="userForm" method="post" action="${pageContext.request.contextPath }/userAction_addUser.action">
			...
			<script type="text/javascript">
				$(function(){
					//发送ajax请求获取所有角色
					$.post('${pageContext.request.contextPath }/roleAction_findAllRole.action',function(list){
						for(var i=0; i<list.length; i++) {
							var role = list[i];
							$("#td-role").append("<input type='checkbox' name='rolesId' id='"+role.id+"' value='"+role.id+"' /><label for='"+role.id+"'>"+role.name+"</label>");
						}
					},'json');
				});
			</script>
